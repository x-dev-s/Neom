...
export async function fetchAllData() {
  try {
    const [rows] = await db.query("SELECT * FROM All_Data"); // Replace with your table name
    return Response.json(rows);
  } catch (error) {
    return null;
  }
...
export const fetchCurtailmentTrendData = async (start, end) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT DATE_FORMAT(Timestamp, '%e/%c/%Y %l:%i:%s %p') AS 'Timestamp', (Calculated_Setpoint/10) AS "Calculated Setpoint", Current_Setpoint AS "Current Setpoint", Actual_Power AS "Actual Power" FROM PV_Data WHERE Timestamp BETWEEN ? AND ?`,
      [start, end]
    );
    return Response.json(rows);
  } catch (error) {
