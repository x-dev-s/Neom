...
export default function DailyYieldBar_SingleGenerator({ id }) {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `/api/data/DailyYieldBar_SingleGenerator?id=${id}&span=${
          span.split(" ")[1]
        }`
      );
      if (!response.ok) {
          ...
          <div className="flex justify-center text-gray-700 text-xs dark:text-gray-400 gap-[6px] items-center relative">
            ...
            <span
              ...
              onClick={() =>
                exportFromJSON({
                  data: data,
                  fileName: `DailyPowerYield_I${id}_${span}`,
