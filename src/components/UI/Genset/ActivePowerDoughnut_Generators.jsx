...
export default function ActivePowerDoughnut_Generators() {
    const [data, setData] = useState([]);

    const fetchData = async () => {
        try {
            const response = await fetch('/api/data/ActivePowerDoughnut_Generators');
            if (!response.ok) {
    ...
    const summary = data.length > 0 ? [
        {
            ...
            share: data[0]["Generator 1"]/ (data[0]["Generator 1"] + data[0]["Generator 2"] + data[0]["Generator 3"]) * 100,
            color: 'bg-blue-500',
        },
        {
            name: 'Generator 2',
            value: data[0]["Generator 2"],
            share: data[0]["Generator 2"]/ (data[0]["Generator 1"] + data[0]["Generator 2"] + data[0]["Generator 3"]) * 100,
            color: 'bg-violet-500'
        },
        {
            name: 'Generator 3',
            value: data[0]["Generator 3"],
            share: data[0]["Generator 3"]/ (data[0]["Generator 1"] + data[0]["Generator 2"] + data[0]["Generator 3"]) * 100,
            color: 'bg-cyan-500'
        },
    ] : [];
...
const customTooltip = (props) => {
    const status = {
        'Generator 1': 'bg-blue-500',
        'Generator 2': 'bg-violet-500',
        'Generator 3': 'bg-cyan-500',
    };
