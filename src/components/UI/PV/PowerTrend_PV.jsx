...
export default function PowerTrend_PV() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/api/data/PowerTrend_PV?start=${start || s}&end=${end || e}`);
      if (!response.ok) {
    ...
    <Card className="h-auto w-full md:h-full sm:mx-auto">
      ...
      <div className="flex flex-wrap justify-between items-center mt-5 w-full max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap justify-stretch w-full gap-1 items-center">
            ...
            <span className="flex flex-1 bg-blue-500 h-full justify-center p-2 rounded-md text-white cursor-pointer dark:bg-blue-500 dark:text-white items-center sm:flex-none" onClick={() => exportFromJSON({ data: data, fileName: `PowerTrend_PV_${start.slice(0,10)}_${end.slice(0,10)}`, exportType: exportFromJSON.types.csv })}>
