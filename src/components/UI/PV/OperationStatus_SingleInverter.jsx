...
export default function OperationStatus_SingleInverter({id}) {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`/api/data/DailyYieldBar_SingleInverter?id=${id}&span=${span.split(" ")[1]}`);
      if (!response.ok) {
      ...
      setData(result.map((item) => ({ 
        ...
        color: colorMapping[item["Daily Power Yield"] > 1 ? "Operational" : "Downtime"],
      })));
  ...
  return (
    <>
      <Card>
        ...
        <Tracker data={data} className="mt-4 w-full flex gap-[1px] rounded text-black dark:text-gray-200 overflow-hidden" />
